		...
		foreach ($data as $key => $value) {
            ...
            if(in_array('deleteProduct', $this->permission)) { 
    			...
    			if ($value['image'] == '<p>You did not select a file to upload.</p>') {
                    $filesrc = base_url('assets/images/product_image/no-image-icon.png');
                } else {
                    $filesrc = base_url($value['image']);
                }
    			
			$img = '<img src="'.$filesrc.'" data-test="'.base_url($value['image']).'" alt="'.$value['name'].'" class="img-circle" width="40" height="40" />';
