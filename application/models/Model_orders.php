...
class Model_orders extends CI_Model
{
	...
	public function create()
	{
        ...
        $total_orders_today = count($this->db->get_where('orders', array('created_date' => date('Y-m-d')))->result_array());
        $newbillno = $total_orders_today + 1;
		//$bill_no = 'INVTB-'.strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 4));
		//$bill_no = 'INVTB'.$newbillno.date('Ymd');
		$bill_no = 'INV'.date('ymdHis');

    	$data = array(
    		'bill_no' => $bill_no,
    		'customer_name' => $this->input->post('customername'),
    		'customer_number' => $this->input->post('customernumber'),
    ...
    // 		'paid_status' => 2,
		...
		foreach($products as $key => $prod) {
		    $items = array(
    			...
    			'qty' => $prod['qty'],
    			'rate' => $prod['rate_value'],
    			'amount' => $prod['amount_value']
    		);
